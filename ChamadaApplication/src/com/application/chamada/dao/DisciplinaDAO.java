...
public class DisciplinaDAO extends AbstractORMLiteHelper<Disciplina>{
	...
	public List<Disciplina> consultarDisciplinasDoDia(DateTime dataAtual){
		...
		QueryBuilder<Disciplina, Integer> queryBuilder = this.getRuntimeDAO().queryBuilder();
		
				
		try{			
			queryBuilder.where().lt(Disciplina.DATA_INICIO, dataAtual).and().gt(Disciplina.DATA_FIM, dataAtual);
